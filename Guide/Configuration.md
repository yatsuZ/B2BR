...
et pour l'activer il faudra aller dans le fichier
...
Attention avec cette deuxième méthode, l'index change après une suppression, verifier après chaque suppression l'index.
...
Essaye de mettre votre code et s'il vous dit accès refusé ou denied, alors c'est bon.
...
PASS_WARN_AGE 7 # Une alerte sera envoyée 7 jour avant la date de changement de mot de passe
```

Ces changement seront appliqués automatiquement mais pas pour l'utilisateur deja crée et le root donc il faudra appliquer ces changement à utilisateur et au root :
